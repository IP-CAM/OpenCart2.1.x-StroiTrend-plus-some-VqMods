...
class ModelCatalogProduct extends Model {
	...
	public function getCities() {
		$query = $this->db->query("SELECT `DescriptionRu`,`Ref` FROM `".DB_PREFIX."novaposhta_cities` ORDER BY `DescriptionRu`");
		/*$query = $this->db->query("
			SELECT `name`, `code`
			FROM `".DB_PREFIX."zone` 
			WHERE `status` = 1 AND `country_id` = 220
			ORDER BY `name`");*/
		return $query->rows;
	}
	...
	public function getProducts_child($data = array()) {
...
/*
	...
	public function getProductDiscounts($product_id, $quantity = 0) {
		$query = $this->db->query("
			SELECT * 
			...
			WHERE product_id = '" . (int)$product_id . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' 
				AND ".(0 == $quantity ? "quantity > 1" : "quantity <= ".(int)$quantity)." AND ((date_start = '0000-00-00' OR date_start < NOW()) 
				AND (date_end = '0000-00-00' OR date_end > NOW())) 
			ORDER BY quantity ".(0 == $quantity ? "ASC" : "DESC").", priority ASC, price ASC
		");
	...
	public function getTotalProducts($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM (SELECT DISTINCT p.sku";
